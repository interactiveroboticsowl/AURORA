<script lang="ts">
	...
	interface Props {
		data: SuperValidated<ItemTemplateSchema>;
	}

	let { data }: Props = $props();
	let form = superForm(data, {
		validators: zodClient(itemTemplateSchema),
		dataType: 'json',
		resetForm: false
	});
...
<div class="flex flex-col h-full">
	<ScrollArea class="flex-grow">
		<form method="POST" use:enhance>
						...
						<Select.Root
							...
							onSelectedChange={(v) => {
								if (v && itemType.parse(v.value)) {
									$formData.item_type = v.value as ItemType;
									if (v.value !== 'question') {
			...
			{#if $formData.item_type === 'question' && $formData.question_type === 'multiple_choice'}
			...
			{#if $formData.item_type === 'static_text'}
				<Form.Field {form} name="text_content">
					<Form.Control let:attrs>
						...
						<Textarea
							bind:value={$formData.text_content as string}
							{...attrs}
