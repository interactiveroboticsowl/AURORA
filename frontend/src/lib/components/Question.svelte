<script lang="ts">
  import { QuestionType } from '$lib/api/itemsApi';
  import type { Question } from '$lib/api/itemsApi';

  export let question: Question;

  let answer: string | number | Record<string, string> = ''; // Adjust to accommodate MatrixScale answers
...
<div class="question mb-6">
  <label for={`question-${question.id}`} class="block text-lg font-semibold mb-2">{question.prompt}</label>
  ...
  {:else if question.question_type === QuestionType.MatrixScale}
    <div class="overflow-x-auto">
      <table class="min-w-full table-auto border border-border rounded-md bg-input text-foreground">
        <thead>
          <tr>
