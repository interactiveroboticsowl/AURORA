<script lang="ts">
	import ProjectSelector from '$lib/components/ProjectSelector.svelte';
	import { Separator } from '$lib/components/ui/separator';
	import * as Resizable from '$lib/components/ui/resizable/index.js';
	import { Edit } from 'lucide-svelte';
	import ItemManagerNav from '$lib/components/ItemManagerNav.svelte';
	import type { LayoutData } from './$types';

	export let data: LayoutData;

	let { projects } = data;
</script>

<Resizable.PaneGroup direction="horizontal" class="h-screen items-stretch">
	<Resizable.Pane defaultSize={20} minSize={15} maxSize={25} class="h-screen">
		<div class="flex h-[52px] items-center justify-between px-4">
			<ProjectSelector {projects} />
			<a href="/item-manager" class="ml-4 text-primary hover:text-accent transition-colors duration-200" title="Manage Items">
				<Edit class="w-6 h-6 hover:text-accent transition-colors duration-200" />
			</a>
		</div>

		<Separator />

		<div class="h-[calc(100vh-52px)] overflow-y-auto">
			<ItemManagerNav />
		</div>
	</Resizable.Pane>

	<Resizable.Handle withHandle />

	<Resizable.Pane class="flex-1 p-4 overflow-y-auto">
		<slot />
	</Resizable.Pane>
</Resizable.PaneGroup>
